...
export default function Sidebar() {
  ...
  useEffect(() => {
    // Calcular mensajes no leídos del Inbox
    const updateUnread = () => {
  ...
  return (
    <aside className="w-64 h-screen bg-slate-900 border-r border-slate-800 flex flex-col shrink-0 text-slate-300">
      ...
      <nav className="flex-grow overflow-y-auto p-4 space-y-1">
        {navItems.map((item, idx) => {
          if (item.type === 'divider') {
            return (
              <div key={idx} className="pt-4 pb-2 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                {item.label}
              </div>
            );
          }
          ...
          return (
            ...
            >
              <i className={`${item.icon} w-5 text-slate-400 group-hover:text-white transition-colors`}></i>
      ...
      <div className="p-4 border-t border-slate-800">
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-semibold text-amber-400 hover:bg-amber-400/5 border border-dashed border-amber-400/20 transition-all"
        >
          <i className="fas fa-key text-amber-500"></i>
          <span>{apiKey ? 'API Key Guardada' : 'Configurar API Key'}</span>
        </button>
      </div>
    </aside>
  );
